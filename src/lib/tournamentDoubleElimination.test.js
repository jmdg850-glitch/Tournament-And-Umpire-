...
import { generateDoubleEliminationBracket, advanceDoubleEliminationBracket } from "./tournamentDoubleElimination.js";
...
describe("advanceDoubleEliminationBracket", () => {
  test("completing the grand final produces zero patches (nothing further to advance)", () => {
    // This is the exact structural signal App.jsx's advanceTournamentBracket relies on
    // to know a double-elimination division's champion was just decided.
  ...
  test("a winners-bracket round-1 loser routes into the losers bracket", () => {
    ...
    // At least one patch should place the loser into the losers bracket match `first` points to.
  ...
  test("phantom-slot cascade terminates for a sparse (N=5-in-8) bracket without throwing", () => {
    ...
    const matches = generateDoubleEliminationBracket(regs(5), { makeId: idGen() });
    // Structural sanity: exactly one final, and every match has a well-formed id/round.
