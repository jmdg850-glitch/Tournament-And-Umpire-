...
- **Offline umpire scoring**: a durable, tested IndexedDB queue plus a local match/score snapshot exist (see Offline Architecture) and cover reload-while-offline, reconnect drain, network flapping, and lost-response retry. Not yet verified: a real Android device/emulator and a real Windows/Electron offline pass — this environment has neither, so treat those as outstanding manual verification, not a known defect.
