...
// Ported from apps/operator/electron/updater.cjs, minus Operator's live
// display-window / live-match restart guards (License Admin has neither).
const {
  autoUpdater,
...
function createDesktopUpdater({ sendToMainWindow }) {
  ...
  function attach() {
    ...
    // The channel is passed in the feed config, never via autoUpdater.channel:
    // that setter also forces allowDowngrade = true in electron-updater.
    try {
