...
export function stationJwtSecret() {
...
export async function signStationJwt({ deviceId, courtId, tournamentId, ttlSec = 3600 }, secret = stationJwtSecret()) {
...
export async function verifyStationJwt(token, secret = stationJwtSecret()) {
...
export async function resolveActor(admin, jwt) {
...
export async function handlePairStation({ admin, body }) {
  ...
  if (body.station_public_id) {
    const { data: court } = await admin.from("courts").select("id, station_public_id").eq("id", grant.court_id).maybeSingle();
    if (!court || court.station_public_id !== String(body.station_public_id).trim()) {
...
async function refreshStation(admin, body) {
  ...
  await admin.from("court_devices").update({ last_seen_at: new Date().toISOString() }).eq("id", device.id);
